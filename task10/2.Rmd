...
Liekanų vidurkis yra lygus beveik 0, nėra koreliacijos tarp liekanų. O normaliojo skirstinio teste gali būti atmesta nulinė hipotezė, kad liekanos neturi normaliojo skirstinio, jei reikšmingumo lygmuo 0,05. Tačiau, jei reikšmingumo lygmuo 0,01, tada negalime tarti, kad liekanos turi normalųjį skirstinį. 
...
Iš histogramos liekanos neatrodo normaliai pasiskirčiusios:
```{r}
plot(fit3$res)
hist(fit3$res, nclass="FD", main="Histogram of residuals")
```

Tačiau palyginę grafikus(baltojo triukšmo grafikas ir liekanų grafikas):
```{r}
par(mfrow=c(1,2))
...
Galime sakyti liekanos yra baltasis triukšmas.