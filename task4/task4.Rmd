...
data=data[-c(208),]#naikiname vieną išskirtį (numeris ir jo padėtis duomenyse nesutapo dėl ankstesnio
                   #duomenių naikinimo)
...
```{r}
bptest(mod4)            # neatmetam H0 hipotezės, kad homoskedastiškas. Reiškia galime teigti, homoskedastiškas
```

Autokoreliacijos tikrinimas:
```{r}
durbinWatsonTest(mod4)  # neatmetam H0 hipotezės, kad nėra autokoreliacijos. Autokoreliacijos nėra 
```
...
Už kiekvieną pločio arba garso izoliacijos rodiklio padidėjimą, kaina padidėja maždaug 600 vienetais. Jei nekilnojamas turtas pirmo aukšto, jo kaina maždaug sumažėja 5000 kainos vienetais. 
